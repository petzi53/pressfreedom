...
**CRAN Submission Ready (2026-08-25):** The pressfreedom package version 0.2.0 is **fully ready for CRAN submission**. All GitHub-only dependencies have been vendored (ggbump, flag-icons); all packages are CRAN-available; R CMD check passes with 0 errors, 0 warnings, 0 notes on all tested platforms (macOS, Windows via winbuilder, multiple platforms via R-hub GitHub Actions v2). A minor informational spelling note on "RWB" from winbuilder is addressed in `cran-comments.md` (RWB is the correct acronym for Reporters Without Borders). See `.posit/assistant/docs/2026-08-25-submission-summary.md` for complete submission status and instructions.
...
Deployment to https://pbaumgartner-pressfreedom.share.connect.posit.cloud/ is **manual, by design** — not automated via GitHub Actions.
...
2. Local `rsconnect` package installed and authenticated via `rsconnect::connectCloudUser()`

**Deployment methods:** See `.posit/assistant/docs/2026-08-07-deployment-guide.md` for multiple deployment approaches (command-line rsconnect, RStudio UI, Positron UI).
...
7. Verify the live app at https://pbaumgartner-pressfreedom.share.connect.posit.cloud/ — check the new year appears in Map/Trends/Country and the About tab's data-year range.
